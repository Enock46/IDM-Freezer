...
def get_base_dir():
    """Get the base directory for storing config and logs."""
    if getattr(sys, 'frozen', False):
        # Running as PyInstaller bundle (.exe) - use AppData
        base = Path(os.getenv('APPDATA')) / "IDM_Manager"
    else:
...
BACKUP_DIR = BASE_DIR / "IDM_Backup"
...
def run_batch_file(parameter="", show_output=True):
    ...
    if getattr(sys, 'frozen', False):
        ...
        batch_file = Path(base_path) / "zied.cmd"
    else:
...
def check_idm_status():
    ...
    try:
        ...
        temp_batch = Path(os.getenv('TEMP')) / "check_idm_temp.bat"
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                temp_batch = Path(os.getenv('TEMP')) / "backup_idm_temp.bat"
...
def check_for_updates():
    ...
    try:
        ...
        result = subprocess.run(
            ['powershell', '-Command',
             "(Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Enock46/IDM-Freezer/main/version.txt' -UseBasicParsing).Content.Trim()"],
            capture_output=True,
            text=True,
            timeout=10
        )
        ...
        if result.returncode == 0:
            ...
            if VERSION == latest_version:
                ...
                print(Fore.CYAN + "Visit https://github.com/Enock46/IDM-Freezer to download the latest version.")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            webbrowser.open("https://github.com/Enock46/IDM-Freezer")
            print(Fore.GREEN + "\n✓ Browser opened!")
